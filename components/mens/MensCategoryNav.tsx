...
import {
  MENS_CATEGORY_GROUPS,
  MENS_RECOMMENDED,
} from "@/lib/data/mens-categories"
...
function CategoryBar() {
  ...
  const active = MENS_CATEGORY_GROUPS.find((g) => g.id === activeId) ?? null
      ...
      <div className="border-y border-[var(--border-dark)]">
        <div className="px-6 lg:px-10">
          {/*
            ...
            The inner grid needs width:100% so it expands to fill the
            container on wide screens — without it, a grid inside an
            overflow wrapper only sizes to its content minimum.
          */}
          <div className="overflow-x-auto" style={{ scrollbarWidth: "none" }}>
            <div
              style={{
                display: "grid",
                gridTemplateColumns: `repeat(${MENS_CATEGORY_GROUPS.length}, 1fr)`,
                width: "100%",       /* ← fills container on desktop */
                minWidth: "800px",   /* ← prevents total collapse on mobile */
              }}
            >
              {MENS_CATEGORY_GROUPS.map((group, idx) => {
                const isActive = group.id === activeId
                const isLast = idx === MENS_CATEGORY_GROUPS.length - 1
                return (
                  <button
                    key={group.id}
                  ...
                  >
                    ...
                    {/* Active gold underline — scales in from center */}
                    <span
      ...
      >
        {active && (
          <div className="px-6 lg:px-10 py-7">
            ...
            {/* Panel header — mirrors the bar's full-width padding */}
            ...
            {/*
              Subcategory grid — equal columns matching the bar above.
              Uses the same column count as the category rail so items
              visually align beneath their parent category.
            */}
            <div
              style={{
                display: "grid",
      ...
      <div className="px-6 lg:px-10">
        <div className="overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          ...
          >
            ...
            {/* Items — equal columns across remaining width */}
            <div
              style={{
                display: "grid",
                gridTemplateColumns: `repeat(${MENS_RECOMMENDED.length}, 1fr)`,
                width: "100%",
              }}
            >
              {MENS_RECOMMENDED.map((item, idx) => {
                const isLast = idx === MENS_RECOMMENDED.length - 1
                return (
                  <Link
                    key={item.slug}
                    href={`/mens/category/${item.slug}`}
...
export default function MensCategoryNav() {
  return (
